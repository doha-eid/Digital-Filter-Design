...
def generate_sig():
    if request.method == 'POST': 
        value_recieved = request.json['y_axis']
        value_array = np.array(value_recieved)
        value = process.apply_filter(value_array)
...
def export_filter():
    if request.method == 'POST':
        ...
        exported_data.to_csv('F:/Digital_filter-main/static/assests/Exported_filter.csv',index = False) 
